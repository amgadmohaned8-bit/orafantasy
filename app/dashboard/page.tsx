...
function DashboardContent({
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
      {/* MY TEAM */}
      <section className="relative overflow-hidden rounded-2xl border border-ora-gold/25 bg-gradient-to-b from-ora-raised to-ora-card shadow-[0_30px_70px_-40px_rgba(0,0,0,0.9)]">
        <div className="ora-nile-band" aria-hidden="true" />

        {/* stadium-spotlight glow behind the title, gives the hero more weight */}
        <div
          aria-hidden="true"
          className="pointer-events-none absolute -left-24 -top-24 h-72 w-72 rounded-full bg-ora-gold/[0.12] blur-3xl"
        />

        <div className="relative p-5 sm:p-7">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <p className="ora-eyebrow">Your squad</p>

              <h1
                className="mt-1 break-words text-[36px] font-black leading-[1.02] tracking-wide sm:text-[42px]"
                style={{
                  ...displayFont,
                  textShadow: "0 2px 24px rgba(201,167,90,0.25)",
                }}
              >
                {profile.teamName || "Build your team"}
              </h1>

              <p className="mt-1.5 text-sm font-semibold text-ora-papyrus/55">
                {profile.coachName
            ...
            <div className="flex shrink-0 items-center gap-3">
              ...
              <Link
                href="/squad"
                className={`inline-flex items-center justify-center rounded-full bg-gradient-to-b from-ora-gold-light to-ora-gold px-5 py-2.5 text-sm font-extrabold text-ora-night shadow-[0_8px_24px_-8px_rgba(201,167,90,0.7)] transition hover:brightness-105 ${focusRing}`}
              >
          ...
          <dl className="mt-7 grid grid-cols-2 gap-3 border-t border-white/5 pt-6 sm:grid-cols-4">
            <Stat label="Squad" value={`${filled}/${SQUAD_SIZE}`} accent="#c9a75a" />
            <Stat
              label={
                pointsLabel !== undefined ? `Gameweek ${pointsLabel}` : "Gameweek"
              }
              value={String(gwPoints)}
              accent="#4fb8c4"
            />
            <Stat label="Season points" value={String(totalPoints)} accent="#e2c778" />
            <Stat
              label="Budget left"
              value={budgetText}
              accent={budgetText.trim().startsWith("-") ? "#e2725a" : "#3fd6a0"}
            />
          </dl>
        </div>
      </section>
    ...
    >
      <div className="flex items-center gap-1.5">
        <span
          ...
          aria-hidden="true"
        />
        <dt className="truncate text-[11px] font-bold uppercase tracking-wide text-ora-papyrus/55">
          {label}
        </dt>
      </div>

      <dd
        className="mt-1.5 text-[32px] font-black leading-none tabular-nums"
